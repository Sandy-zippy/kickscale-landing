...
const MONTHS = [
  { label: 'Today', cycles: 15, status: 'baseline' as const },
  { label: 'Month 1', cycles: 28, status: 'in-progress' as const },
  { label: 'Month 2', cycles: 44, status: 'in-progress' as const },
  { label: 'Month 3', cycles: 60, status: 'goal' as const },
]
const MAX = 60
      ...
      <div className="aspect-[4/3] w-full p-6 sm:p-8 flex flex-col">
        <motion.div initial={{ opacity: 0, y: 8 }} animate={inView ? { opacity: 1, y: 0 } : { opacity: 0 }} transition={{ duration: 0.4 }} className="mb-6">
          <div className="flex items-baseline gap-2 mb-1.5">
            <div className="text-[10px] sm:text-xs tracking-[0.18em] uppercase font-bold text-[#2A2A35]/60">Your procedure trajectory</div>
          ...
          <div className="text-2xl sm:text-3xl md:text-4xl font-bold leading-[1.05] text-[#2A2A35]" style={{ fontFamily: '"Space Grotesk", sans-serif', letterSpacing: '-0.02em' }}>
            ...
            <span className="text-[#D5EB4B]"><AnimatedCounter target={60} /></span>{' '}
            <span className="text-base sm:text-lg font-normal text-[#5A5A66]">procedures / month</span>
          </div>
          <div className="text-sm text-[#5A5A66] mt-1">
            +45 LASIK + cataract procedures by Day 90. <span className="text-[#2A2A35] font-semibold">Same ad spend.</span>
          </div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 8 }} animate={inView ? { opacity: 1, y: 0 } : { opacity: 0 }} transition={{ duration: 0.5, delay: 1.8 }} className="px-4 py-3 rounded-lg bg-[#2A2A35] text-[#FFFDF7] flex items-center gap-3">
          ...
          <div className="text-xs sm:text-sm leading-tight">
            <span className="text-[#D5EB4B] font-bold font-mono">+₹45L revenue</span>{' '}
            <span className="text-[#FFFDF7]/70">at avg ₹1L procedure · paid for 22× over by sprint end</span>
          </div>
        </motion.div>
      </div>
