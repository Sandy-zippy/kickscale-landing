...
const PAINS = [
  { text: '87 unread shared inbox', x: 8, y: 18 },
  { text: 'Two agents replied to same RFQ', x: 56, y: 14 },
  { text: 'Quote takes 36 hours', x: 12, y: 38 },
  { text: 'P&L arrives day 8', x: 50, y: 34 },
  { text: 'Tally rec 8 hrs/mo', x: 6, y: 60 },
  { text: 'Vendor rebates never chased', x: 54, y: 56 },
  { text: 'Fridays late at office', x: 18, y: 80 },
  { text: 'Friday partner reports manual', x: 50, y: 78 },
]

const CYCLE_MS = 9000

export default function HeroCorporateTravel() {
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      ...
      if (elapsed < 5000) {
        setCounter(Math.round((elapsed / 5000) * 87))
      } else if (elapsed < 6500) {
        setCounter(Math.round(87 - ((elapsed - 5000) / 1500) * 87))
      } else {
        setCounter(0)
      }
      frame++
  ...
  return (
    <div
      ...
      role="img"
      aria-label="A corporate travel agency's 87-unread shared bookings inbox is auto-assigned. Quote turnaround drops from 36 hours to 4. P&L visible day 2 not day 8."
    >
      <div
      ...
      <div className="absolute top-3 left-3 right-3 flex items-center justify-between text-[10px] font-mono text-[#6B7280] z-30">
        <span className="flex items-center gap-1.5">
          <motion.span
            ...
            aria-hidden="true"
          />
          bookings@ inbox · {isResolved ? 'Mon 9 AM' : 'Mon 9:14 AM'}
        </span>
        <span className="opacity-70">{isResolved ? 'Auto-assigned' : `${counter} unread`}</span>
      </div>
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="font-bold text-sm leading-tight">Quote in 4 hours. P&L on day 2. Rebates auto-claimed.</p>
