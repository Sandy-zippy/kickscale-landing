...
const PAINS = [
  { text: '47 IG DMs unread', x: 8, y: 18 },
  { text: 'Friday tables lost', x: 56, y: 14 },
  { text: 'Stock chaos in 3 groups', x: 12, y: 38 },
  { text: '2-day Zomato rec', x: 50, y: 34 },
  { text: 'Repeat customers unknown', x: 6, y: 60 },
  { text: 'Birthdays never captured', x: 54, y: 56 },
  { text: 'Walk-ins not tracked', x: 18, y: 80 },
  { text: 'Marketing ₹45K/mo no ROI', x: 50, y: 78 },
]

const CYCLE_MS = 9000

export default function HeroHospitality() {
  ...
  return (
    <div
      ...
      role="img"
      aria-label="A cafe owner's 47 unread Instagram DMs become a unified reservation inbox. Zomato reconciliation drops from 2 days to 90 minutes. Repeat customers detected automatically."
    >
      <div
      ...
      <div className="absolute top-3 left-3 right-3 flex items-center justify-between text-[10px] font-mono text-[#6B7280] z-30">
        <span className="flex items-center gap-1.5">
          <motion.span
            ...
            aria-hidden="true"
          />
          IG + WhatsApp + diary · {isResolved ? 'Mon 10 AM' : 'Wed 8 AM'}
        </span>
        <span className="opacity-70">{isResolved ? 'One inbox' : `${counter} IG DMs`}</span>
      </div>
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="font-bold text-sm leading-tight">Zomato rec in 90 min. Friday tables saved.</p>
