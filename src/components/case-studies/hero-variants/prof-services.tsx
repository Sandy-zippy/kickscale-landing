...
const PAINS = [
  { text: 'Lead in email, lost', x: 8, y: 18 },
  { text: 'Proposal takes a week', x: 56, y: 14 },
  { text: 'Partner is the bottleneck', x: 12, y: 38 },
  { text: 'DSO 60+ days', x: 50, y: 34 },
  { text: 'Pipeline invisible', x: 6, y: 60 },
  { text: 'Calendly back-and-forth', x: 54, y: 56 },
  { text: 'Friday status updates manual', x: 18, y: 80 },
  { text: 'Knowledge in partner\'s head', x: 50, y: 78 },
]
...
export default function HeroProfServices() {
  ...
  return (
    <div
      ...
      role="img"
      aria-label="A consulting firm partner's email + LinkedIn + WhatsApp unify into a single pipeline. Proposals draft in a day instead of a week. DSO drops from 60+ to under 35."
    >
      <div
      ...
      <div className="absolute top-3 left-3 right-3 flex items-center justify-between text-[10px] font-mono text-[#6B7280] z-30">
        <span className="flex items-center gap-1.5">
          <motion.span
            ...
            aria-hidden="true"
          />
          Senior partner inbox · {isResolved ? '9:00 AM' : 'Friday 5 PM'}
        </span>
        <span className="opacity-70">{isResolved ? 'Pipeline visible Mon' : `${counter} unread`}</span>
      </div>
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="font-bold text-sm leading-tight">Proposal in a day. DSO under 35. Partner unblocked.</p>
