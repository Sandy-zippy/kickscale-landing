...
const PAINS = [
  { text: 'Counsellor 1 offline', x: 8, y: 18 },
  { text: 'Lead waited 40min', x: 56, y: 14 },
  { text: '5 WhatsApps no sync', x: 12, y: 38 },
  { text: 'Demo missed: counsellor on leave', x: 50, y: 34 },
  { text: 'Fee reminder forgotten', x: 6, y: 60 },
  { text: 'Dropout flagged month 3', x: 54, y: 56 },
  { text: '30% conversion gap', x: 18, y: 80 },
  { text: '₹1.5L Meta wasted/mo', x: 50, y: 78 },
]
...
export default function HeroCoaching() {
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      ...
      if (elapsed < 5000) {
        setCounter(Math.round((elapsed / 5000) * 53))
      } else if (elapsed < 6500) {
        setCounter(Math.round(53 - ((elapsed - 5000) / 1500) * 53))
      } else {
        setCounter(0)
      }
      frame++
  ...
  return (
    <div
      ...
      role="img"
      aria-label="A coaching institute owner's 5 counsellor WhatsApps unify into a single inbox, leads stop slipping, conversion lifts 30%."
    >
      <div
      ...
      <div className="absolute top-3 left-3 right-3 flex items-center justify-between text-[10px] font-mono text-[#6B7280] z-30">
        <span className="flex items-center gap-1.5">
          <motion.span
            ...
            aria-hidden="true"
          />
          5 counsellor phones · {isResolved ? '9:00 AM' : '10:42 AM'}
        </span>
        <span className="opacity-70">{isResolved ? '1 inbox' : `${counter} leads waiting`}</span>
      </div>
      ...
      <motion.div
        ...
        aria-hidden="true"
      >
        <span className="text-[9px] uppercase tracking-widest opacity-80 leading-none mb-0.5">Waiting</span>
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="font-bold text-sm leading-tight">+30% conversion. 5 inboxes to 1.</p>
