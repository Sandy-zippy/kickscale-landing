...
const PAINS = [
  { text: 'Meta lead waits 3h', x: 8, y: 18 },
  { text: '₹2K cart abandons daily', x: 56, y: 14 },
  { text: '10h/wk on personal DMs', x: 12, y: 38 },
  { text: 'Bitespeed didn\'t deliver', x: 50, y: 34 },
  { text: 'Founder bottleneck', x: 6, y: 60 },
  { text: 'VIPs nudged manually', x: 54, y: 56 },
  { text: 'Attribution post-iOS broken', x: 18, y: 80 },
  { text: 'Replenishment sequence missing', x: 50, y: 78 },
]

const CYCLE_MS = 9000

export default function HeroD2c() {
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      ...
      if (elapsed < 5000) {
        setCounter(Math.round((elapsed / 5000) * 38))
      } else if (elapsed < 6500) {
        setCounter(Math.round(38 - ((elapsed - 5000) / 1500) * 38))
      } else {
        setCounter(0)
      }
      frame++
  ...
  return (
    <div
      ...
      role="img"
      aria-label="A D2C founder's 6-hour lead-to-touch becomes 5 minutes. Cart recovery from 4% to 12%. Founder DM hours from 15 to 3."
    >
      <div
      ...
      <div className="absolute top-3 left-3 right-3 flex items-center justify-between text-[10px] font-mono text-[#6B7280] z-30">
        <span className="flex items-center gap-1.5">
          <motion.span
            ...
            aria-hidden="true"
          />
          Founder's phone · {isResolved ? '6:00 AM' : '11:38 PM'}
        </span>
        <span className="opacity-70">{isResolved ? 'CX handles it' : `${counter} DMs unread`}</span>
      </div>
      ...
      <motion.div
        ...
        aria-hidden="true"
      >
        <span className="text-[9px] uppercase tracking-widest opacity-80 leading-none mb-0.5">DMs</span>
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="font-bold text-sm leading-tight">Cart recovery 12%+. Founder reclaims 12 hrs/week.</p>
