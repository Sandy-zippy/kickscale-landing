...
const PAINS = [
  { text: 'Whiteboard photo at 8pm', x: 8, y: 18 },
  { text: '40 distributors, 1 group', x: 56, y: 14 },
  { text: '8h/wk Excel→Tally typing', x: 12, y: 38 },
  { text: 'Month-end 3 days minimum', x: 50, y: 34 },
  { text: 'Weighbridge data lost', x: 6, y: 60 },
  { text: 'Distributor statements manual', x: 54, y: 56 },
  { text: 'Sunday on accounts', x: 18, y: 80 },
  { text: 'Sister-concern transfers untracked', x: 50, y: 78 },
]
...
export default function HeroRiceMill() {
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      ...
      if (elapsed < 5000) {
        setCounter(Math.round((elapsed / 5000) * 72))
      } else if (elapsed < 6500) {
        setCounter(Math.round(72 - ((elapsed - 5000) / 1500) * 72))
      } else {
        setCounter(0)
      }
      frame++
  ...
  return (
    <div
      ...
      role="img"
      aria-label="A rice mill founder's whiteboard, 40-distributor WhatsApp group, weighbridge, and Tally all flow into a single ledger. Month-end shrinks from 3 days to 1."
    >
      <div
      ...
      <div className="absolute top-3 left-3 right-3 flex items-center justify-between text-[10px] font-mono text-[#6B7280] z-30">
        <span className="flex items-center gap-1.5">
          <motion.span
            ...
            aria-hidden="true"
          />
          Plant + 40 distributors · {isResolved ? '8:00 PM' : '8:47 PM'}
        </span>
        <span className="opacity-70">{isResolved ? 'Synced to Tally' : `${counter} unread`}</span>
      </div>
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="font-bold text-sm leading-tight">Whiteboard stays. Month-end shrinks 3 days to 1.</p>
