...
const PAINS = [
  { text: 'MR orders on WhatsApp photos', x: 8, y: 18 },
  { text: '5-day month-end', x: 56, y: 14 },
  { text: '5-8% SKU errors weekly', x: 12, y: 38 },
  { text: 'Credit limits skipped', x: 50, y: 34 },
  { text: 'GSTR mismatches 10%', x: 6, y: 60 },
  { text: 'Near-expiry write-offs', x: 54, y: 56 },
  { text: 'Sunday rec sessions', x: 18, y: 80 },
  { text: 'Raju won\'t change his ways', x: 50, y: 78 },
]

const CYCLE_MS = 9000

export default function HeroPharma() {
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      ...
      if (elapsed < 5000) {
        setCounter(Math.round((elapsed / 5000) * 5))
      } else if (elapsed < 6500) {
        setCounter(Math.round(5 - ((elapsed - 5000) / 1500) * 5))
      } else {
        setCounter(0)
      }
      frame++
  ...
  return (
    <div
      ...
      role="img"
      aria-label="A pharma C&F distributor's 5-day month-end shrinks to 1 day. MR field orders captured cleanly. GSTR auto-reconciled."
    >
      <div
      ...
      <div className="absolute top-3 left-3 right-3 flex items-center justify-between text-[10px] font-mono text-[#6B7280] z-30">
        <span className="flex items-center gap-1.5">
          <motion.span
            ...
            aria-hidden="true"
          />
          MR field + admin desk · {isResolved ? 'Day 1' : 'Day 5'}
        </span>
        <span className="opacity-70">{isResolved ? 'Closed' : `${counter} days month-end`}</span>
      </div>
      ...
      <motion.div
        ...
        aria-hidden="true"
      >
        <span className="text-[9px] uppercase tracking-widest opacity-80 leading-none mb-0.5">Days</span>
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="font-bold text-sm leading-tight">Month-end in 1 day. SKU errors under 1%.</p>
