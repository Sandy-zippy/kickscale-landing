...
const PAINS = [
  { text: 'Quote takes 2 days', x: 8, y: 18 },
  { text: '800 chats locked on his phone', x: 56, y: 14 },
  { text: 'ERP NX disconnected from sales', x: 12, y: 38 },
  { text: 'Customer asks status, no one knows', x: 50, y: 34 },
  { text: 'PO entry lag 3 days', x: 6, y: 60 },
  { text: 'Foreman won\'t log into NX', x: 54, y: 56 },
  { text: 'Production WhatsApp chaos', x: 18, y: 80 },
  { text: '₹8L ERP under-used', x: 50, y: 78 },
]
...
export default function HeroHvac() {
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      ...
      if (elapsed < 5000) {
        setCounter(Math.round((elapsed / 5000) * 36))
      } else if (elapsed < 6500) {
        setCounter(Math.round(36 - ((elapsed - 5000) / 1500) * 36))
      } else {
        setCounter(0)
      }
      frame++
  ...
  return (
    <div
      ...
      role="img"
      aria-label="An HVAC manufacturer's quote turnaround drops from 2 days to 4 hours. Sales head's 800 chats migrate to CRM. ERP NX finally talks to sales."
    >
      <div
      ...
      <div className="absolute top-3 left-3 right-3 flex items-center justify-between text-[10px] font-mono text-[#6B7280] z-30">
        <span className="flex items-center gap-1.5">
          <motion.span
            ...
            aria-hidden="true"
          />
          Sales head's phone · {isResolved ? '9:00 AM' : '11:18 AM'}
        </span>
        <span className="opacity-70">{isResolved ? 'NX synced' : `${counter} hr quote lag`}</span>
      </div>
      ...
      <motion.div
        ...
        aria-hidden="true"
      >
        <span className="text-[9px] uppercase tracking-widest opacity-80 leading-none mb-0.5">Hr lag</span>
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="font-bold text-sm leading-tight">Quote in 4 hours. NX bridged. Foreman keeps WhatsApp.</p>
