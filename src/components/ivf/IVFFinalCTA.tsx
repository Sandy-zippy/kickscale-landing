...
export default function IVFFinalCTA() {
  return (
    <section className="bg-[#FFFDF7] relative overflow-hidden py-32 sm:py-40 px-6 border-t border-[#2A2A35]/10">
      {/* Lime ambient glow */}
      <div
        aria-hidden
        className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[80%] h-[80%] rounded-full opacity-15 pointer-events-none"
        style={{ background: '#D5EB4B', filter: 'blur(120px)' }}
      />
      ...
      >
        <div className="text-xs tracking-[0.18em] uppercase font-bold text-[#2A2A35]/60 mb-6">
          Two founding slots remaining
        </div>
        ...
        <p className="text-lg text-[#5A5A66] max-w-2xl mx-auto mb-10 leading-relaxed">
          Free audit. Custom cycle-volume target. ₹2L locked forever for the first 3 Hyderabad clinics. <span className="text-[#2A2A35] font-semibold">We refund you if we don&apos;t deliver.</span>
        </p>
        <div className="flex flex-wrap justify-center gap-3">
          <WhatsAppCTA variant="primary" label="WhatsApp us → 30-min reply" trackingLabel="final_wa" />
          <a
            href="#hero-form"
            data-tracking="final_apply"
            onClick={(e) => {
              e.preventDefault()
              window.scrollTo({ top: 0, behavior: 'smooth' })
            }}
            className="inline-flex items-center gap-2 px-6 py-3 rounded-md font-medium border-2 border-[#2A2A35] text-[#2A2A35] hover:bg-[#2A2A35] hover:text-[#FFFDF7] transition-colors"
          >
