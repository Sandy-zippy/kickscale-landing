...
const WEBHOOK_URL = import.meta.env.VITE_HAIR_WEBHOOK_URL || 'https://sandyautomations.app.n8n.cloud/webhook/hair-lp'

const VOLUME_OPTIONS = [
  '<5 transplants / mo (early-stage)',
  '5–15 transplants / mo (growing)',
  '15–30 transplants / mo (established)',
  '30+ transplants / mo (chain / large clinic)',
]
...
export default function HeroForm() {
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    const payload = {
      vertical: 'hair-transplant', source: 'hero_form', city: 'Hyderabad',
      clinic_name: fd.get('clinic_name'), whatsapp: fd.get('whatsapp'),
      monthly_volume: fd.get('monthly_volume'), urgency: fd.get('urgency'),
    ...
    try {
      ...
      console.error('Hair form submit error', err)
  ...
  return (
    <motion.form initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.3 }} onSubmit={handleSubmit} className="rounded-2xl border-2 border-[#2A2A35] bg-[#FFFDF7] p-5 sm:p-6 space-y-3.5 shadow-[0_20px_60px_rgba(42,42,53,0.12)]">
      ...
      <div>
        ...
        <input id="clinic_name" name="clinic_name" required className={fieldClass} placeholder="e.g., Assure Clinic, Banjara Hills" />
      </div>
      <div>
      ...
      <div>
        <label htmlFor="monthly_volume" className={labelClass}>Transplants booked each month today</label>
