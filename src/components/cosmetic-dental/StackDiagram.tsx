...
const PIPELINE = [
  { stage: 'Acquisition', tools: ['Stape', 'Meta + Google Ads'], benefit: 'Bid on case starts, not selfie submissions. Recover the 25 to 40 percent of cases Meta misses post-iOS14.' },
  { stage: 'Conversion', tools: ['GHL', 'n8n', 'EMI partners'], benefit: 'AI smile preview qualifies. WhatsApp follow-up sequence. Pre-approved EMI before the chair.' },
  { stage: 'Creative', tools: ['Higgsfield', 'Nano Banana', 'Claude AI'], benefit: '8 to 12 fresh smile-makeover ad variants weekly. DCI-compliant by default. Call analysis every week.' },
]
      ...
      <div className="relative max-w-6xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-100px' }} transition={{ duration: 0.5 }} className="grid grid-cols-1 md:grid-cols-12 gap-8 mb-20 items-baseline">
          ...
          <h2 className="md:col-span-9 text-4xl sm:text-5xl lg:text-6xl font-bold leading-[1.02] tracking-tight">
            Three stages. <span className="text-[#FFFDF7]/50">From ad rupee to booked smile makeover.</span>
          </h2>
        </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-80px' }} transition={{ duration: 0.5 }} className="grid grid-cols-1 md:grid-cols-[80px_1fr] gap-6 sm:gap-10 py-10 border-t border-[#FFFDF7]/15">
            ...
            <div className="md:pt-3">
              ...
              <p className="text-2xl sm:text-3xl font-bold leading-tight" style={{ fontFamily: '"Space Grotesk", sans-serif' }}>
                Booked smile makeovers. Predictably. <span className="text-[#D5EB4B]">Every month.</span>
              </p>
            </div>
          </motion.div>
        </div>
