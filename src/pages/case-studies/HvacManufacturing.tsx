...
import { content } from './content/hvac'

const SLUG = 'hvac-manufacturing-po-to-production-automation' as const
const meta = registry[SLUG]

const beforeMessages = [
  { sender: 'Pune builder', body: '30 TR AHU quote needed urgent', time: '9:14 AM' },
  { sender: 'Hyd pharma', body: 'Following up on last week\'s enquiry', time: '9:15 AM' },
  { sender: 'Production WA', body: 'Bracket 4mm shortage, what to do?', time: '10:42 AM' },
  { sender: 'UAE EPC', body: 'Status of order #4471?', time: '11:08 AM' },
  { sender: 'AutoCAD designer', body: 'Drawing ready, where to send?', time: '11:30 AM' },
]
const afterMessages = [
  { sender: 'AI', body: '✓ Quote drafted from BOM library. Sent for partner review.', time: 'Just now' },
  { sender: 'AI', body: '✓ Order #4471: in production, ETA Friday. Status sent.', time: 'Just now' },
  { sender: 'AI', body: '⚠ Routed to production foreman: bracket shortage flagged.', time: 'Just now' },
]

export default function HvacManufacturing() {
  return (
    <CaseStudyLayout metadata={meta}>
      <CaseStudyHero
        ...
        headline={meta.title}
        subhead="The middleware layer for HVAC and air-solutions manufacturers whose ERP doesn't touch sales, quoting, or production handoff."
        statBefore={meta.heroStatBefore}
        statAfter={meta.heroStatAfter}
        statLabel={meta.heroStatLabel}
        benefits={[
          'Quote drafted in 30 min, not 6 hours',
          'NX work-order status visible to customer',
          'Foreman keeps WhatsApp, ERP gets clean data',
        ]}
      ...
      <section id="pain" className="py-16 px-4 bg-white border-y border-[#E5E7EB]">
        <div className="max-w-6xl mx-auto grid lg:grid-cols-[1.2fr_1fr] gap-12 items-center">
          <div>
            <ScrollReveal>
              <h2 className="text-3xl font-bold text-[#1A1A2E] mb-3">A week from enquiry to dispatch.</h2>
              <p className="text-[#4B5563] leading-relaxed mb-6">₹8L of ERP. Sales still on the founder's personal WhatsApp. Production runs on voice notes.</p>
              <ul className="space-y-3 mb-6">
                {[
                  { time: '9 AM', label: 'Enquiries on sales head\'s personal WhatsApp + email.' },
                  { time: 'Morning', label: 'Manual quote in Excel from reference BOM (6h per quote).' },
                  { time: 'Afternoon', label: 'AutoCAD designer consulted, drawing emailed back.' },
                  { time: 'Day 2', label: 'PO arrives email, forwarded to production WhatsApp group.' },
                  { time: 'Day 3', label: 'Foreman asks clarifications via voice notes, sales relays.' },
                  { time: 'Week 2', label: 'Customer chases status, sales head answers personally.' },
                ].map((row, i) => (
              ...
              <p className="text-[#1A1A2E] font-semibold leading-snug border-l-4 border-[#D5EB4B] pl-4">
                The owner does not need to rip out NX. He needs the bridge that makes NX finally talk to sales, customers, and production WhatsApp.
              </p>
            </ScrollReveal>
          </div>
          <div>
            <WhatsAppChaosMockup beforeMessages={beforeMessages} afterMessages={afterMessages} profile={{ name: 'PO Intake · Sales', initials: 'PO' }} />
          </div>
        </div>
      </section>
      ...
      <section className="py-20 px-4 bg-[#FFFDF7]">
        <FeaturedQuote
          ...
          context="From a discovery call with a Hyderabad HVAC manufacturer, March 2026"
        />
      </section>
      ...
      <section id="before" className="py-16 px-4">
        <div className="max-w-4xl mx-auto">
          <ScrollReveal>
            ...
            <p className="text-[#6B7280] mb-6">The 10 manual steps a manufacturer does (or skips) every single week.</p>
          </ScrollReveal>
          <BeforeStateMap
            steps={content.beforeStateSteps}
            totalSummary={{ value: '~32 hrs/week', label: 'lost across sales, AutoCAD, and production to manual quoting, customer status calls, and PO clarification voice notes. Per factory. Every week.' }}
          />
        </div>
      </section>
      ...
      <section className="py-12 px-4 bg-white border-y border-[#E5E7EB]">
        <div className="max-w-4xl mx-auto bg-[#FFFDF7] border-l-4 border-[#D5EB4B] rounded-xl p-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <p className="font-semibold text-[#1A1A2E]">Want this stack for your factory? Book your audit.</p>
          <a
            href="#audit-form"
          ...
          <ScrollReveal delay={0.1}>
            <div className="mb-10">
              <StackArchitectureDiagram
                ariaLabel="A customer enquiry flows through the AI quote drafter into GHL, which pushes confirmed POs to ERP NX via the n8n bridge. The production foreman uses a WhatsApp bot to mark status, which syncs back to NX and triggers customer status updates."
                nodes={[
                  { id: 'customer', label: 'Customer', sublabel: 'WhatsApp + email', x: 8, y: 50, variant: 'source' },
                  { id: 'ai', label: 'AI Quote drafter', sublabel: 'BOM library', x: 28, y: 50, variant: 'middle' },
                  { id: 'ghl', label: 'GHL CRM', sublabel: '8-stage pipeline', x: 50, y: 50, variant: 'middle' },
                  { id: 'nx', label: 'NX bridge', sublabel: 'n8n + REST/CSV', x: 72, y: 22, variant: 'middle' },
                  { id: 'foreman', label: 'Foreman bot', sublabel: 'WhatsApp buttons', x: 72, y: 50, variant: 'middle' },
                  { id: 'docs', label: 'Doc auto', sublabel: 'PO ack + dispatch', x: 72, y: 78, variant: 'middle' },
                  { id: 'sales', label: 'Sales head', sublabel: 'unblocked', x: 92, y: 30, variant: 'human' },
                  { id: 'shipped', label: 'Order shipped', sublabel: 'NX synced · customer informed', x: 92, y: 70, variant: 'sink' },
                ]}
                edges={[
                  { from: 'customer', to: 'ai', delay: 0 },
                  { from: 'ai', to: 'ghl', delay: 0.15 },
                  { from: 'ghl', to: 'nx', delay: 0.3 },
                  { from: 'ghl', to: 'foreman', delay: 0.4 },
                  { from: 'ghl', to: 'docs', delay: 0.5 },
                  { from: 'nx', to: 'sales', delay: 0.65 },
                  { from: 'docs', to: 'shipped', delay: 0.75 },
                  { from: 'foreman', to: 'shipped', delay: 0.85 },
                ]}
              />
            </div>
          </ScrollReveal>
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <WhatYouGetTable
            oneTime={{
              ...
              items: [
                { included: true, label: 'Half-day shadow at sales desk + production floor', detail: 'We sit with your sales head and walk the shop floor. Map every quote, every PO, every status call.' },
                { included: true, label: 'GHL sub-account fully configured', detail: 'Pipeline (8 stages), custom fields, BOM library, conversation inbox.' },
                { included: true, label: 'WhatsApp Business API on sales head\'s number', detail: 'Port his existing 800-chat number behind shared inbox. Customers see no change.' },
                { included: true, label: '8 Meta-approved WhatsApp templates', detail: 'Enquiry ack, quote-ready, quote-revised, PO-acknowledged, in-production, dispatched, invoice-shared, payment receipt.' },
                { included: true, label: 'AI quote drafter trained on your BOM library', detail: 'Reads enquiry PDF, matches reference BOM, applies fee matrix. 6-hour quote becomes 30-minute review.' },
                { included: true, label: 'n8n bridge to ERP NX (REST or CSV fallback)', detail: 'Confirmed PO pushes to NX as work order. Status syncs back every 30 minutes.' },
                { included: true, label: 'Production foreman WhatsApp bot', detail: 'Foreman marks Started / Done / Blocked via single-tap WhatsApp. NX gets clean data.' },
                { included: true, label: 'Documented SOPs + 90-min team training', detail: 'Hand-over docs your sales engineer, AutoCAD designer, and foreman can actually follow.' },
            ...
            recurring={{
              ...
              items: [
                ...
                { included: true, label: 'AI quote drafter inference + tuning', detail: 'Monthly retraining as your product variants expand.' },
                { included: true, label: 'n8n + NX bridge hosting + monitoring', detail: 'We watch the queue. We get paged when something breaks.' },
                { included: true, label: 'Email infrastructure', detail: 'Customer status, dispatch advice, invoice copies.' },
      ...
      <section id="vs" className="py-16 px-4 bg-white border-y border-[#E5E7EB]">
        <div className="max-w-5xl mx-auto">
          ...
          <ComparisonTable
            columns={[
              { name: 'Siemens NX add-on', badge: '₹3L+/yr' },
              { name: 'Bitespeed / Interakt', badge: 'WABA SaaS' },
              { name: 'Hire CRM consultant', badge: '6-mo project' },
              { name: 'ZippyScale', badge: 'Done-for-you', isUs: true },
            ]}
            rows={[
              {
                feature: 'AI quote drafter',
                values: [
                  { check: 'no', text: 'Manual quoting' },
                  { check: 'no', text: 'Out of scope' },
                  { check: 'partial', text: 'You build templates' },
                  { check: 'yes', text: 'BOM library + fee matrix' },
                ],
              },
              {
                feature: 'NX bridge for sales',
                values: [
                  { check: 'partial', text: 'Heavy customization' },
                  { check: 'no', text: 'Out of scope' },
                  { check: 'partial', text: 'Bespoke build, slow' },
                  { check: 'yes', text: 'n8n REST + CSV fallback' },
                ],
              },
              {
                feature: 'Foreman without screen',
                values: [
                  { check: 'no', text: 'NX login required' },
                  { check: 'no', text: 'Out of scope' },
                  { check: 'partial', text: 'Custom build' },
                  { check: 'yes', text: 'WhatsApp bot, single-tap' },
                ],
              },
              {
                feature: 'Customer auto-status',
                values: [
                  { check: 'no', text: 'Sales head walks floor' },
                  { check: 'partial', text: 'You build flow' },
                  { check: 'partial', text: 'They build it' },
                  { check: 'yes', text: 'WA on each stage' },
                ],
              },
              {
              ...
              },
              {
                ...
                values: [
                  '₹25K+ + your time',
                  '₹5K-15K + your time',
                  '₹2-5L project + retainer',
                  '₹16K, fully managed',
                ],
              },
              {
                feature: 'Knowledge persists if sales head leaves',
                values: [
                  { check: 'partial', text: 'NX retains it' },
        ...
        <section id="faq" className="py-16 px-4">
          <div className="max-w-3xl mx-auto">
            <ScrollReveal>
              ...
              <p className="text-[#6B7280] mb-8">The questions every manufacturer asks before signing the audit. No fluff.</p>
            </ScrollReveal>
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <AuditTimeline
            steps={[
              ...
              {
                hour: '24',
                title: 'We map your sales-to-production handoff',
                description: 'Half-day shadow at your factory if you are in Hyderabad / Pune / Chennai, remote walkthrough otherwise. We screenshot your sales head\'s WhatsApp, your NX work-order screen, your AutoCAD shared folder.',
              },
              {
                hour: '48',
                title: 'You get the audit deliverable',
                description: 'A short PDF: the 3 biggest workflow leaks we found, target outcomes per leak, and the exact 4-week sprint scope priced at ₹2L. No fluff, no upsell.',
              },
              {
                hour: '∞',
